defmodule RetroWeb.MemberControllerTest do
  ...
  describe "POST /rooms/:room_id/members/invite" do
    test "it sends email invite to list of recipients and creates members", %{conn: conn} do
      ...
      post conn, "api/rooms/#{room.id}/members/invite", %{emails: "fake@example.com, fake2@example.com"}
  ...
  describe "DELETE /rooms/:room_id/members/:id" do
    test "it sends email invite to list of recipients and creates members", %{conn: conn} do
      {:ok, room} = Room.create(%{name: "Dev Retro", password: "bethcatlover"})
      {:ok, member} = Repo.insert(Member.changeset(%Member{email: "member@ex.com", room_id: room.id}))
      ...
      delete conn, "api/rooms/#{room.id}/members/#{member.id}"
